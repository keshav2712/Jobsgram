        ...
        .then(res => {
          this.setState({
            ...
            for (let i = 0; i < res.data.skills.length; i++) {
              defSkills[i] = { value: res.data.skills[i], label: res.data.skills[i] }
            }
      ...
      return this.state.education.map((el, i) => 
          ...
          <div className="row" style={{marginBottom: "0px"}}>
            <div className="input-field col s12" style={{margin: "7px 0"}} key={i}>
                ...
                <input 
                  ...
                  className={classnames("", {
                        invalid: errors.education[i] ? errors.education[i].institutionName : null
                      })}
                  onChange={this.handleChange.bind(this, i)}
                />
                <span className="red-text">{errors.education[i] ? errors.education[i].institutionName : null}</span>
            </div>
          </div>
            <div className="row" style={{marginBottom: "0px"}}>
              <div className="input-field col s4">
                ...
                onChange={this.handleChange.bind(this, i)}
                className={classnames("", {
                        invalid: errors.education[i] ? errors.education[i].startYear : null
                      })}
              />
                <span className="red-text">{ errors.education[i] ? errors.education[i].startYear : null}</span>
            </div>
            <div className="input-field col s4 offset-s1">
                ...
                onChange={this.handleChange.bind(this, i)}
                className={classnames("", {
                        invalid: errors.education[i] ? errors.education[i].endYear : null
                      })}
              />
                <span className="red-text">{errors.education[i] ? errors.education[i].endYear : null}</span>
            </div>
            <div className="col s3 valign-wrapper" style={{height: '79px'}}>
              {this.state.disabled ? null : <input type='button' className="btn btn-small" style={{backgroundColor: 'red', margin: "0 auto"}} value='X' onClick={this.removeClick.bind(this, i)}/>}
            </div>
          </div>
          </React.Fragment> 
      )
                ...
                <form onSubmit={this.onSubmit}>
                  ...
                  <div className="row">
                    <div className="col s10 offset-s2" style={{paddingTop: "10px"}}>
                      {this.createUI()}        
                      {this.state.disabled ? null : <input className="btn btn-small" disabled = {this.state.disabled} type='button' value='add more' onClick={this.addClick.bind(this)}/>}
                    </div>
                  </div>
